...
import by.jrr.jdbc.bean.Company;
...
public class DeveloperRunner {
    ...
    public static void main(String[] args) {
        ...
        developerRunner.updateForDenisAndVova();
    ...
    public void updateForDenisAndVova(){
        Session session = sessionFactory.openSession();
        Transaction transaction = null;

        transaction = session.beginTransaction();
        List<Company> companies = session.createQuery("FROM Company").list();
        updateDeveloper(3, companies.get(0));
    }
    ...
    public void updateDeveloper(int developerId, Company experience) {
        Session session = sessionFactory.openSession();
        Transaction transaction = null;

        transaction = session.beginTransaction();
        Developer developer = (Developer) session.get(Developer.class, developerId);
        developer.setCompany(experience);
